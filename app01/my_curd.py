...
class UserCurdAdmin(core_func.BaseCurdAdmin):
    ...
    def reverse_produce_url(self, model_obj=None, is_header=False):
        ...
        if is_header:
            ...
            # 取model_name的方式
            print(type(model_obj), '---type?', model_obj.__dict__, type(model_obj)._meta.model_name,
                  self.model_class._meta.model_name)
            # 取app_label的方式
            print(type(model_obj), '---type?', model_obj.__dict__, type(model_obj)._meta.app_label,
                  self.model_class._meta.app_label)
            # 获取namespace及app_name,model_name的方式1,使用type获取model_obj类，然后回去该类的属性
            print(type(model_obj), '---type?', model_obj.__dict__, type(model_obj)._meta.model_name,
                  type(model_obj)._meta.app_label)
    ...
    list_display = ['id', 'username', 'email', 'role', 'usergroup', edit, delete, reverse_produce_url,
                    combine_username_email]
